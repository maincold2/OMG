...
from gaussian_renderer import render_imp, render_depth#, render_simp
import sys
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration == opt.iterations:
                ...
                gaussians.decode(save_dict, decompress=False)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration % args.depth_reinit_interval==0:
                    pts, rgb = gaussians.depth_reinit(scene, render_depth, iteration, args.num_depth, args, pipe, background)
            ...
            if iteration == args.simp_iteration1:
                xyz, rgb = gaussians.intersection_sampling(scene, render_imp, iteration, args, pipe, background)
                
                gaussians.max_sh_degree=dataset.sh_degree
                gaussians.reinitial_pts(xyz, rgb)
            ...
            if iteration == args.simp_iteration2:
                ...
                imp_score = gaussians.intersection_preserving(scene, render_imp, iteration, args, pipe, background)
                gaussians.ld_scoring(imp_score, args.importance_thresh, args.lambda_ld)
                gaussians.training_setup(opt)
                torch.cuda.empty_cache()   
